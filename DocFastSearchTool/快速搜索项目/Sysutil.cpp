...
void DirectoryList(const string& path,vector<string>& subfile, vector<string>& dirfile)
{
	...
	{
		cout << e << endl;
		vector<string> _dirfile, _subfile;
		DirectoryList(path + "\\" + e, _dirfile, _subfile);
	}
	*/
	_findclose(handle);
...
void SystemEnd()
{
	SetCurPos(((Width - 4 - strlen("Çë°´ÈÎÒâ¼ü¼ÌÐø. . .")) / 2), Height - 1);
}
...
void DrawFrame()
{
	...
	system(str);
	system("color 0F");
	Draw_H_W(Width, Height);
}
void DrawMenu()
{
	DrawFrame();
	SetCurPos(((Width - 4 - strlen(title)) / 2), 1);
	printf("DocFastSerachTool");

	SetCurPos(2, 3);
	printf("%-50s%-65s", "Ãû×Ö", "Â·¾¶");

	SetCurPos(((Width - 4 - strlen("exitÍË³ö.")) / 2), Height - 3);
	printf("exitÍË³ö.");
...
void ColourPrintf(const string  &str)
{
	...
	GetConsoleScreenBufferInfo(handle, &csbi);   //°×É«
	colorOld = csbi.wAttributes;

	SetConsoleTextAttribute(handle, color);      //ºìÉ«

	printf("%s", str.c_str());
	SetConsoleTextAttribute(handle, colorOld);   //°×É«
}
